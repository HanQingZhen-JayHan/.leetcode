...
public:
    NumMatrix(vector<vector<int>>& matrix) {
        int n = matrix.size();
        int m = matrix[0].size();
        data = vector<vector<int>>(n + 1, vector<int>(m + 1, 0));
        for(int r = 1; r <= n; r++) {
            for(int c = 1; c <= m; c++) {
                data[r][c] = matrix[r - 1][c - 1] + data[r][c - 1] +
    ...
    int sumRegion(int row1, int col1, int row2, int col2) {
        return data[row2 + 1][col2 + 1] + data[row1][col1] -
        data[row2 + 1][col1] - data[row1][col2 + 1];
    }
};
