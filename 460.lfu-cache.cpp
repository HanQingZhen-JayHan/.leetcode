/*
 * @lc app=leetcode id=460 lang=cpp
 *
 * [460] LFU Cache
 *
 * https://leetcode.com/problems/lfu-cache/description/
 *
 * algorithms
 * Hard (39.52%)
 * Likes:    5227
 * Dislikes: 314
 * Total Accepted:    223.6K
 * Total Submissions: 515.1K
 * Testcase Example: '["LFUCache","put","put","get","put","get","get","put","get","get","get"]\n' +
  '[[2],[1,1],[2,2],[1],[3,3],[2],[3],[4,4],[1],[3],[4]]'
 *
 * Design and implement a data structure for a Least Frequently Used (LFU)
 * cache.
 *
 * Implement the LFUCache class:
 *
 *
 * LFUCache(int capacity) Initializes the object with the capacity of the data
 * structure.
 * int get(int key) Gets the value of the key if the key exists in the cache.
 * Otherwise, returns -1.
 * void put(int key, int value) Update the value of the key if present, or
 * inserts the key if not already present. When the cache reaches its capacity,
 * it should invalidate and remove the least frequently used key before
 * inserting a new item. For this problem, when there is a tie (i.e., two or
 * more keys with the same frequency), the least recently used key would be
 * invalidated.
 *
 *
 * To determine the least frequently used key, a use counter is maintained for
 * each key in the cache. The key with the smallest use counter is the least
 * frequently used key.
 *
 * When a key is first inserted into the cache, its use counter is set to 1
 * (due to the put operation). The use counter for a key in the cache is
 * incremented either a get or put operation is called on it.
 *
 * The functions get and put must each run in O(1) average time complexity.
 *
 *
 * Example 1:
 *
 *
 * Input
 * ["LFUCache", "put", "put", "get", "put", "get", "get", "put", "get", "get",
 * "get"]
 * [[2], [1, 1], [2, 2], [1], [3, 3], [2], [3], [4, 4], [1], [3], [4]]
 * Output
 * [null, null, null, 1, null, -1, 3, null, -1, 3, 4]
 *
 * Explanation
 * // cnt(x) = the use counter for key x
 * // cache=[] will show the last used order for tiebreakers (leftmost element
 * is  most recent)
 * LFUCache lfu = new LFUCache(2);
 * lfu.put(1, 1);   // cache=[1,_], cnt(1)=1
 * lfu.put(2, 2);   // cache=[2,1], cnt(2)=1, cnt(1)=1
 * lfu.get(1);      // return 1
 * ⁠                // cache=[1,2], cnt(2)=1, cnt(1)=2
 * lfu.put(3, 3);   // 2 is the LFU key because cnt(2)=1 is the smallest,
 * invalidate 2.
 * // cache=[3,1], cnt(3)=1, cnt(1)=2
 * lfu.get(2);      // return -1 (not found)
 * lfu.get(3);      // return 3
 * ⁠                // cache=[3,1], cnt(3)=2, cnt(1)=2
 * lfu.put(4, 4);   // Both 1 and 3 have the same cnt, but 1 is LRU, invalidate
 * 1.
 * ⁠                // cache=[4,3], cnt(4)=1, cnt(3)=2
 * lfu.get(1);      // return -1 (not found)
 * lfu.get(3);      // return 3
 * ⁠                // cache=[3,4], cnt(4)=1, cnt(3)=3
 * lfu.get(4);      // return 4
 * ⁠                // cache=[4,3], cnt(4)=2, cnt(3)=3
 *
 *
 *
 * Constraints:
 *
 *
 * 1 <= capacity <= 10^4
 * 0 <= key <= 10^5
 * 0 <= value <= 10^9
 * At most 2 * 10^5 calls will be made to get and put.
 *
 *
 *
 *
 */

// @lc code=start
#include "BaseSolution.h"
#include "PrintTools.h"
#include <list>
#include <string>
#include <tuple>
#include <unordered_map>
#include <utility>
class LFUCache : public BaseSolution {
public:
    void test() {
        put(1, 1);
        put(2, 2);
        printOutput(get(1));
        put(3, 3);
        printOutput(get(2));
        printOutput(get(3));
        put(4, 4);
        printOutput(get(1));
        printOutput(get(3));
        printOutput(get(4));
    }

private:
    // key: <key>, value: <fre,value,iterator>
    unordered_map<int, tuple<int, int, list<int>::iterator>> cache;
    // key: <fre>, value: keylist[iterator, iterator,...]
    unordered_map<int, list<int>> frequencies;
    // capacity
    int capacity;
    // minimum frequency
    int min;

    void insert(int key, int value, int fre) {
        frequencies[fre].push_back(key);
        cache[key] = { fre, value, --frequencies[fre].end() };
    }

public:
    LFUCache(int capacity) : capacity(capacity), min(0) {}

    int get(int key) {
        const auto it = cache.find(key);
        if(it == cache.end()) {
            return -1;
        }
        const int fre = std::get<0>(it->second);
        const int value = std::get<1>(it->second);
        const auto iter = std::get<2>(it->second);
        // remove old frequency
        frequencies[fre].erase(iter);
        // update minimum frequency if no more
        if(frequencies[fre].empty()) {
            frequencies.erase(fre);
            if(fre == min) {
                ++min;
            }
        }
        // insert new frequency
        insert(key, value, fre + 1);

        return value;
    }

    void put(int key, int value) {

        auto it = cache.find(key);
        // find the key
        if(it != cache.end()) {
            std::get<1>(it->second) = value;
            get(key);
            return;
        }
        // not find the key
        if(cache.size() >= capacity) {
            int minFreKey = frequencies[min].front();
            cache.erase(minFreKey);
            frequencies[min].pop_front();
            if(frequencies[min].empty()) {
                frequencies.erase(min);
            }
        }
        min = 1;
        insert(key, value, 1);
    }
};

/**
 * Your LFUCache object will be instantiated and called as such:
 * LFUCache* obj = new LFUCache(capacity);
 * int param_1 = obj->get(key);
 * obj->put(key,value);
 */
// @lc code=end
