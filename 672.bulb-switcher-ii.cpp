...
class Solution672 : public BaseSolution {
public:
    void test() { printOutput(flipLights(3, 1000)); }
...
public:
    int flipLights(int n, int presses) {
        ...
        dfs(bulbs, status, presses,dp);
        print(status);
        return status.size();
    }
    string op(string bulbs, int start, int step) {

        for(int i = start; i < bulbs.size(); i += step) {
            if(bulbs[i] == '1') {
                bulbs[i] = '0';
            } else {
                bulbs[i] = '1';
    ...
    unordered_map<string, unordered_set<int>>& dp) {
        ...
        if(dp[bulbs].count(press))
            return;

        dp[bulbs].insert(press);
        --press;
        // btn1
        dfs(op(bulbs, 0, 1), status, press, dp);

        // btn2 even
        dfs(op(bulbs, 1, 2), status, press, dp);

        // btn3 odd
        dfs(op(bulbs, 0, 2), status, press, dp);

        // btn4
        dfs(op(bulbs, 0, 3), status, press, dp);
    }
};
// @lc code=end
